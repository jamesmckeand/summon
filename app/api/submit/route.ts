...
async function validateArtist(name: string): Promise<{ validated: boolean; image: string | null }> {
  const artist = await searchArtist(name);
  return { validated: !!artist, image: artist?.image ?? null };
}
...
export async function POST(request: Request) {
  ...
  if (type === "artist") {
    ...
    const { validated, image } = await validateArtist(artist_name.trim());
