                  ...
                  <Link href={`/explore?city=${encodeURIComponent(c.city)}`}>
                    <div className="glass rounded-xl p-4 hover:border-primary/20 transition-all group">
                      <div className="flex items-center gap-4">
                        <span className="text-muted-foreground text-sm font-mono w-6 text-center shrink-0">
                          {i + 1}
                        </span>
                        <div className="w-10 h-10 rounded-xl gradient-brand flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                          ...
                          <div className="flex items-center justify-between mt-1">
                            {c.topArtist && (
                              <p className="text-xs text-muted-foreground truncate">
                                Top: <span className="text-foreground">{c.topArtist}</span>
                              </p>
                            )}
                            {next && (
                              <p className="text-xs text-muted-foreground shrink-0 ml-auto">
                                {(next - c.totalVotes).toLocaleString()} to next threshold
                              </p>
                            )}
                          </div>
                        </div>
